...
class ScalingCoefficients:
    ...
    def __init__(self, str: float, dex: float, faith: float, luck: float, int: float) -> None:
        self.str = str
        self.dex = dex
        self.int = int
        self.faith = faith
...
class Infusion:
    ...
    def physical_blessed(self) -> bool:
        return (
            self.infusion == WeaponInfusion.BLESSED
            or self.weapon.name in {
    ...
    def damages(self, str: int, dex: int, int_: int, faith: int, luck: int) -> Tuple[int, int, int, int, int]:
        requirements = self.weapon.requirements
        if (str < requirements.str
        ...
        return (
            int(self.damage.physical * (
                    ...
                    + (
                        ...
                        if self.physical_blessed else
                        0
                    )
            )),
            int(self.damage.magic * (
                    1
                    + self.scaling.int * self.saturation.magic[int_]
                    + (
                        self.scaling.faith * self.saturation.magic[faith]
                        if self.magic_blessed else
                        0
                    )
            )),
            int(self.damage.fire * (
                    1
                    + self.scaling.int * self.saturation.fire[int_]
                    + self.scaling.faith * self.saturation.fire[faith]
            )),
            int(self.damage.lightning * (
                    1
                    + self.scaling.faith * self.saturation.lightning[faith]
            )),
            int(self.damage.dark * (
                    1
    ...
    def max_level(self, levels, points):
        values = sorted(
            self.level(levels, points),
            key=lambda i: sum(i[0]),
            reverse=True
        )
        max_ar = None
        for value in values:
